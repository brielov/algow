...
const tPairOf = (a: Type, b: Type): Type => tapp(tapp(tPair, a), b);
...
const inferFieldAccess = (
	...
	const [subst, recordType, constraints] = inferExpr(
		env,
		registry,
		expr.record,
	);
...
const inferApp = (
	...
	const [s2, paramType, c2] = inferExpr(
		applySubstEnv(s1, env),
		registry,
		expr.param,
	);
...
const inferBinOp = (
	...
	const [s2, rightType, c2] = inferExpr(
		applySubstEnv(s1, env),
		registry,
		expr.right,
	);
...
const inferIf = (
	...
	const [s3, thenType, c2] = inferExpr(
		applySubstEnv(subst, env),
		registry,
		expr.then,
	);
	subst = composeSubst(subst, s3);
	const [s4, elseType, c3] = inferExpr(
		applySubstEnv(subst, env),
		registry,
		expr.else,
	);
...
const inferTuple = (
	...
	let resultType = types[types.length - 1]!;

	for (let i = types.length - 2; i >= 0; i--) {
		resultType = tPairOf(types[i]!, resultType);
	}
...
const inferPattern = (
	...
	switch (pattern.kind) {
		case "PVar": {
			return [
				subst,
				new Map([[pattern.name, applySubst(subst, expectedType)]]),
			];
		}
		case "PWildcard": {
...
const inferMatch = (
	...
	if (missing.length > 0) {
		throw new TypeError(
			`Non-exhaustive patterns. Missing: ${missing.join(", ")}`,
		);
	}
...
export const processDataDecl = (
	decl: ast.DataDecl,
): [TypeEnv, ConstructorRegistry] => {
...
export const baseEnv: TypeEnv = new Map([
	...
	[
		...
		scheme(
			...
			tfun(
				tfun(tvar("a"), tvar("b")),
				tfun(tListOf(tvar("a")), tListOf(tvar("b"))),
			),
		),
	],
	...
	[
		"tail",
		scheme(["a"], tfun(tListOf(tvar("a")), tMaybeOf(tListOf(tvar("a"))))),
	],
	...
	[
		...
		scheme(
			["a"],
			tfun(tListOf(tvar("a")), tfun(tListOf(tvar("a")), tListOf(tvar("a")))),
		),
	],
	...
	[
		...
		scheme(
			...
			tfun(
				tfun(tvar("a"), tBool),
				tfun(tListOf(tvar("a")), tListOf(tvar("a"))),
			),
		),
	],
