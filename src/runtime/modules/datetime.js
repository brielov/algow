...
$foreign.DateTime = {
  ...
  add: (duration) => (dt) => {
    const ts = $foreign.DateTime.toTimestamp("UTC")(dt);
    return $foreign.DateTime.fromTimestamp("UTC")(ts + duration);
  },
  ...
  diff: (a) => (b) => {
    const tsA = $foreign.DateTime.toTimestamp("UTC")(a);
    const tsB = $foreign.DateTime.toTimestamp("UTC")(b);
    return tsB - tsA;
  },
  ...
  fromIso: (str) => {
    ...
    const dateResult = $foreign.Date.make(year)(month)(day);
    if (dateResult[0] === 0) return [0];

    const timeResult = $foreign.Time.make(hour)(minute)(second);
    if (timeResult[0] === 0) return [0];

    return [1, { date: dateResult[1], time: timeResult[1] }];
  },

  format: (zone) => (locale) => (dt) => {
    const ts = $foreign.DateTime.toTimestamp(zone)(dt);
    const d = new Date(ts);
